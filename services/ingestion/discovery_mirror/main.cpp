...
DEFINE_string(kafka_topic, "ifex.discovery.201", "Kafka topic to consume");
...
int main(int argc, char* argv[]) {
    ...
    while (!g_shutdown) {
        ...
        auto offboard_msg = ifex::offboard::decode_discovery_offboard(msg.value);
        if (!offboard_msg) {
            LOG(WARNING) << "Failed to decode discovery offboard message";
            messages_failed++;
            continue;
        }
